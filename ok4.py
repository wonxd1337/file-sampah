# bot.py - Versi minimalis dengan tabel bersih
import subprocess
import time
import requests
import re
...
def draw_table(stdscr, packages, status_map, username_map, cmd_map):
    ...
    # Table header
    header = f"{'#':<3} {'Username':<20} {'Package':<25} {'Status':<8} {'Cmd':<8}"
    ...
    # Footer - hanya separator
    footer_y = max_rows + 3
    if footer_y < h - 1:
        stdscr.addstr(footer_y, 0, "-" * min(w-1, 70))
        stdscr.addstr(footer_y + 1, 0, "Press 'q' or ESC to exit")
    
    stdscr.refresh()

def process_commands(commands, cmd_map):
    """Process commands and update cmd_map"""
    ...
    while True:
        ...
        if now - last_update >= interval:
            ...
            for pkg in packages:
                ...
                status_map[pkg] = "Online" if running else "Offline"
                username_map[pkg] = last_status["username"].get(pkg, "Unknown")
                if pkg not in cmd_map:
                    cmd_map[pkg] = "IDLE"
            
            # Check for commands
            commands = get_pending_commands()
            if commands:
                process_commands(commands, cmd_map)
