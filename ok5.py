# bot.py - Versi dengan tampilan tabel bersih (mempertahankan fungsionalitas asli)
import subprocess
import time
import requests
import re
...
# Simpan command terakhir yang dieksekusi
last_commands = {}
...
# ==================== UI TABEL DENGAN CURSES ====================
def draw_table(stdscr, packages, status_map, username_map, cmd_map):
    curses.curs_set(0)
    stdscr.clear()
    h, w = stdscr.getmaxyx()
    ...
    # Table header
    header = f"{'#':<3} {'Username':<18} {'Package':<25} {'Status':<8} {'Cmd':<8}"
    stdscr.addstr(0, 0, header[:w-1], curses.A_BOLD)
    stdscr.addstr(1, 0, "-" * min(w-1, 68))
    
    # Table content
    max_rows = min(len(packages), h - 4)
    for i in range(max_rows):
        pkg = packages[i]
        uname = username_map.get(pkg, "Unknown")
        stat = status_map.get(pkg, "Offline")
        cmd = cmd_map.get(pkg, "IDLE")
        is_online = stat == "Online"
        color = curses.color_pair(1) if is_online else curses.color_pair(2)
        
        # Shorten names
        uname_short = uname[:16] + ".." if len(uname) > 16 else uname
        pkg_short = pkg[:23] + ".." if len(pkg) > 23 else pkg
        
        line = f"{i+1:<3} {uname_short:<18} {pkg_short:<25}"
        status_text = "ON" if is_online else "OFF"
        stdscr.addstr(i+2, 0, line[:w-1])
        stdscr.addstr(i+2, len(line), f" {status_text:^6}", color)
        stdscr.addstr(i+2, len(line) + 8, f" {cmd:<8}")
...
def monitor_curses(stdscr):
    ...
    while True:
        ...
        # Sync status setiap 2 detik (sama seperti interval di versi asli)
        time.sleep(2)
        ...
        # Check commands setiap 5 loop (sama seperti di versi asli)
        if loop_count % 5 == 0:
        ...
        # Draw table
        draw_table(stdscr, packages, status_map, username_map, cmd_map)
