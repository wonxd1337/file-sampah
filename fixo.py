...
def get_running_packages():
    ...
    for pkg in installed_packages:
        # PINTU GERBANG: Cek apakah aplikasi jalan (Terbuka/Tertutup)
        pid_out = run_root(f"pidof {pkg}")
        
        if pid_out: 
            first_pid = pid_out.split()[0]
            
            # DETEKTOR GAME: Cek UDP
            udp_check = run_root(f"netstat -unp 2>/dev/null | grep {first_pid}/")
            if udp_check.strip():
                running_info[pkg] = 2  # State 2 = In Game (Online)
            else:
                running_info[pkg] = 1  # State 1 = Terbuka, tapi di Lobby (Opened)
