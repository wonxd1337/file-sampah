...
# print_status is kept but all calls are commented out to avoid any output
def print_status(message, type="info"):
    pass  # no-op, but kept for potential debugging
...
def sync_status():
    ...
    # status change detection (no print)
    # We keep the logic to update last_status
...
def main():
    clear_screen()
    sync_status()  # initial sync, no output
    ...
    while True:
        try:
            ...
            # Print the table with current status and pending commands
            ...
            if isinstance(commands, dict) and commands:
                for pkg, cmd_info in commands.items():
                    ...
                    if cmd == "START":
                        ...
                        ack_execution(pkg)  # unknown command, still ack
